...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V1_conv_inp = layers.Conv2D(64, kernel_size=7, strides=(1,1), padding="same", use_bias=False)
        self.V1_conv_fb1 = layers.Conv2DTranspose(64, kernel_size=7, strides=(4,4), padding="same", use_bias=False)
        self.V1_conv_fb2 = layers.Conv2DTranspose(64, kernel_size=7, strides=(8,8), padding="same", use_bias=False)
        self.V1_conv_lat = layers.Conv2D(64, kernel_size=7, padding="same", use_bias=False)
        self.V1_mp = layers.MaxPooling2D(pool_size=(4,4), strides=(4,4), padding="same")
        ...
        self.V2 = RecurrentCNNBlock(128, times=2, use_fb=True, us_factor=2)
        self.mp1 = layers.MaxPooling2D(pool_size=(2,2), strides=(2,2), padding="same")
        self.V4 = RecurrentCNNBlock(256, times=4, use_fb=True, us_factor=2, pool_factor=8)
    ...
    def call(self, x, training=None, mask=None):
        ...
        for t in range(self.fb_loops-1):
            x = self.V1_Add([
                v1_init_response,
                self.V1_conv_lat(v1_state),
                self.V1_conv_fb1(self.V2.hidden_state),
